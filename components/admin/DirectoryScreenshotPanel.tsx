...
import {
  adminBackfillScreenshots,
  adminResetFailedScreenshots,
} from "@/app/admin/directory/actions";
import type { ScreenshotJobResult } from "@/lib/directory/screenshotJob";

const BATCH_OPTIONS = [10, 25, 50, 100];

type Props = {
  /** Listings already carrying a screenshot. */
  done: number;
  /** Listings never attempted. */
  pending: number;
  failed: number;
};

/**
 * Runs the screenshot backfill in bounded batches.
 *
 * Each attempt costs a Firecrawl credit, so the spend is always an explicit
 * click with a visible batch size rather than one job that runs until the
 * balance is gone.
 */
export function DirectoryScreenshotPanel({ done, pending, failed }: Props) {
  const [busy, startTransition] = useTransition();
  const [batch, setBatch] = useState(25);
  const [result, setResult] = useState<ScreenshotJobResult | null>(null);
  ...
  function run() {
    ...
    startTransition(async () => {
      setResult(await adminBackfillScreenshots({ limit: batch }));
    });
  }
  ...
  function retryFailed() {
    ...
    setResetMsg(null);
    startTransition(async () => {
      const res = await adminResetFailedScreenshots();
      setResetMsg(
        res.error
          ? res.error
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-brand-gray-300">
            {done} captured · {pending} waiting
            {failed > 0 ? ` · ${failed} failed` : ""}. Each listing in a batch
            costs one Firecrawl credit. Safe to stop and continue later.
          </p>
        </div>

        <div className="flex flex-wrap items-center gap-2">
          <label className="sr-only" htmlFor="screenshot-batch">
            Batch size
          </label>
          <select
            id="screenshot-batch"
        ...
        <div className="mt-4 space-y-2 rounded-xl border border-white/10 bg-black/30 p-4">
          {result.error ? (
            ...
            <p className="text-sm font-semibold text-emerald-300">
              {result.captured} captured, {result.failed} failed,{" "}
              {result.skipped} skipped (no website).{" "}
