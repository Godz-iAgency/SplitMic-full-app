...
| 12 | Directory card imagery | **Derived favicon logos (free) + Firecrawl website screenshots (1 credit each, batched).** The scrape captured no images, so they have to be generated. Logos come from Google's favicon endpoint at render time — nothing stored, no key, no credits — and screenshots are backfilled incrementally into the existing `profile-media` bucket. Cards fall back to a brand gradient so a partial backfill never looks broken. |
...
- [ ] **Capture listing screenshots** — `/admin/directory` → "Generate screenshots", in batches. One Firecrawl credit per listing attempted; ~505 listings have websites. Resumable, so stop and continue whenever.
