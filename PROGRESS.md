...
Vitest, 424 tests across `lib/**/*.test.ts`, run with `npm test`. Covers the
...
| 14a-5 | No-photo card image | **SplitMic's own generic venue photo (`/players/venue.jpg`, `CATEGORY_META.venue.image`), not an abstract placeholder.** An abstract glow/pattern treatment (ported from the directory's `BusinessCard` fallback art) was tried first and shipped, then replaced after the user asked for the directory's actual generic venue photo instead — it's the same asset already used on the landing page's Venues tile and the directory's Venues category page, so a no-photo card still reads as "us," not filler. `EventImageBand.tsx` now always renders an `<img>` (real photo or the generic one), with no separate no-image branch. |
...
1. **Uber deep link no longer pre-fills on desktop web.** The Uber URL format (`m.uber.com/ul/?action=setPickup&dropoff[...]`, now built once in `lib/events/getThereLinks.ts`) is unchanged and still matches Uber's documented scheme, but testing it directly in a real browser — with an address, with coordinates, with/without `action=setPickup` — every variant now 301s to Uber's plain marketing homepage with pickup/dropoff both empty. This isn't a param bug on our side; Uber's own web fallback appears to have stopped honoring guest prefill. The same URL may still work correctly on a phone with the Uber app installed, since iOS/Android intercept that link at the OS level before it ever reaches Uber's servers — unverifiable from this environment. **Needs a real phone test with the Uber app installed** to know whether this is a desktop-only degradation or broken everywhere. If it's broken everywhere, the real fix requires registering an Uber developer `client_id`, which is an account-creation decision, not a code change. (The new no-match chooser, #14a-4, surfaces the same broken link — it doesn't fix the underlying Uber-side prefill issue, just gives it a second, more honest entry point.)
