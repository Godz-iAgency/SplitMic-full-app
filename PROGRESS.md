...
| 12 | Directory card imagery | **Three sources, tried in order: Google Places venue photo → Firecrawl website screenshot → brand gradient.** The scrape captured no images at all. A Places photo is the real shot of the business and is what a visitor actually wants; a website screenshot is the fallback when Google has no match. Logos are separate and free — derived from each site's favicon at render time, nothing stored. Both photo pipelines are batched, resumable, and never re-attempt a row, since each attempt is billed. |
...
- [ ] **Run `migrations/step13_directory_places.sql`** and **enable "Places API (New)"** on the existing Google Maps key in Google Cloud Console, then `/admin/directory` → "Fetch venue photos" in batches. This is what puts real venue photos on the cards.
- [ ] **Capture listing screenshots** — `/admin/directory` → "Generate screenshots", in batches. One Firecrawl credit per listing attempted; ~505 listings have websites. Only a fallback now: cards prefer a Google venue photo when one exists.
