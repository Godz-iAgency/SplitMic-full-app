...
Vitest, 414 tests across `lib/**/*.test.ts`, run with `npm test`. Covers the
...
| 14a-4 | /live cards without a real destination | **Superseded — see below.** Originally shipped as "no link, not a generic Google search," then reversed after seeing it live: an unclickable card among clickable ones reads as broken, not as "no data available," and the user experience cost outweighed the precision concern. Now every card links to a Google Maps search for the venue name (`lib/directory/media.ts`'s `reviewsUrl`, the same helper the directory's own Reviews button already uses) as the last resort after a SplitMic profile match and a directory match. Known rough edge: a handful of Do512 rows carry a town name instead of a venue name (e.g. "Luckenbach, TX"), and the search term unconditionally appends ", Austin, TX", producing a harmless but odd doubled locality string. Not worth a special case for the rare out-of-Austin show. |
...
1. **Uber deep link no longer pre-fills on desktop web.** `GetThereButtons`' Uber URL format (`m.uber.com/ul/?action=setPickup&dropoff[...]`) is unchanged and still matches Uber's documented scheme, but testing it directly in a real browser — with an address, with coordinates, with/without `action=setPickup` — every variant now 301s to Uber's plain marketing homepage with pickup/dropoff both empty. This isn't a param bug on our side; Uber's own web fallback appears to have stopped honoring guest prefill. The same URL may still work correctly on a phone with the Uber app installed, since iOS/Android intercept that link at the OS level before it ever reaches Uber's servers — unverifiable from this environment. **Needs a real phone test with the Uber app installed** to know whether this is a desktop-only degradation or broken everywhere. If it's broken everywhere, the real fix requires registering an Uber developer `client_id`, which is an account-creation decision, not a code change.
