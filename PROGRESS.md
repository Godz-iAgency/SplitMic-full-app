...
| 14a-4 | /live cards without a real destination | **No link, not a generic Google search.** A card's whole-card click target is: matched SplitMic venue profile, else matched directory listing, else nothing clickable. Considered falling back to a Google search by venue name for the unmatched case; rejected — a search result isn't reliably "this venue," and a card that looks clickable but lands somewhere wrong is worse than one that plainly isn't. |
